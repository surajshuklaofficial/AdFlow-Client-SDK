...
import { CalendarDateRangePicker } from "../../../common/dashboard/date-range-picker";
import { Overview } from "../../../common/dashboard/overview";
import { RecentSales } from "../../../common/dashboard/recent-sales";
...
import { deleteAdAsync, updateAdAsync } from "../../slice";
...
export function SelectCampaignState() {
    ...
    <>
      ...
      {value === "update" && (
        <div>
          ...
          <div className="fixed inset-0 bg-white opacity-90 z-30" onClick={() => handleCampaignStatusChange("active")}/>
        </div>
      )}
    </>
  );
...
export function DeleteConfirmation({ handleCampaignStatusChange }) {
  const { id } = useParams();
  const dispatch = useDispatch();
